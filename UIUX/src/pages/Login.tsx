...
export default function Login() {
  ...
  const accounts: Record<string, { path: string; label: string }> = {
    "admin": { path: "/admin", label: "Pengelola KIP-K" },
    "mahasiswa": { path: "/mahasiswa", label: "Mahasiswa" },
    "prodi": { path: "/prodi", label: "Program Studi" },
    "warek": { path: "/warek", label: "Warek III" },
  };
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (acc && password === "kip2026") {
        navigate(acc.path);
      } else {
        setError("NIM atau password salah. Coba: admin/mahasiswa/prodi/warek + kip2026");
      }
      setLoading(false);
    }, 800);
  };
          ...
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i}
              ...
              style={{
                ...
                bottom: `-${(i + 1) * 30}px`, right: `-${(i + 1) * 20}px`,
              }}
            />
          ))}
        </div>
          ...
          <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-100">
            ...
            <div className="flex flex-wrap gap-2">
              {Object.entries(accounts).map(([key, { label }]) => (
                <button key={key}
                  onClick={() => { setUsername(key); setPassword("kip2026"); }}
