...
export default function ProdiMahasiswaList() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError("");
    getProdiMahasiswaList(buildFilter())
  ...
  async function handleDelete() {
    ...
    try {
      ...
      setDeleteModal(null);
      setDeleteConfirmNim("");
      const res = await getProdiMahasiswaList(buildFilter());
      setStudents(res.data || []);
      setTotal(res.total || 0);
      setTotalPages(Math.max(1, res.totalPages || 1));
    } catch (err: any) {
    ...
    <div className="space-y-3 sm:space-y-4 w-full max-w-7xl mx-auto min-w-0" onClick={() => setOpenMenu(null)}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 min-w-0">
        ...
        <div className="flex flex-col min-[480px]:flex-row min-[480px]:items-center gap-2 w-full sm:w-auto shrink-0">
          ...
          />
          <a
            href={`${import.meta.env.VITE_API_URL ?? "http://localhost:8000/api"}/ekspor/mahasiswa/download?tahun_ajaran=${tahunAjaranFilter}`}
          ...
          <table className="w-full min-w-[960px] text-sm">
            <thead>
              <tr className="bg-[#F8FAFC] border-b border-[#E2E8F0]">
                {["No", "NIM", "Nama", "Angkatan", "Kategori", "IPK", "Progres IPS", "Semester", "Status", "SP", "Aksi"].map(
                  (h) => (
                    <th
                      key={h}
